...
function handlePOST(req, res){
     if(req.url =='/post'){
        console.log('do you want to post?');
        req.on('data', function(chunk){
            console.log('parsed:', chunk);
            var dataFilePath = './data/' + randomStringAsBase64Url(fileSize);
            while(fs.existsSync(dataFilePath)){ // find a way to do this asynchronously ? 
                dataFilePath = './data/' + randomStringAsBase64Url(fileSize);
            }
            fs.writeFile(dataFilePath, chunk, function(err) {
                if(err) {
                    console.log(err);
                } else {
        ...
        req.on('data', function(chunk){
            ...
            console.log('aid recieved:' + data['aid']);
            var dataFilePath = './data/' + data['aid'];
...
function handlePlay(req, res, querystring){
    ...
    console.log('handlePlay:' + article_id);
...
//    console.log(JSON.stringify(str_query));
